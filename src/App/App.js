...
class App extends React.Component {
  ...
  componentDidMount() {
    ...
    Promise.all([projectsResponse, tasklistsResponse, tasksResponse]).then((responses) => {
      return Promise.all(responses.map((response) => {
        return response.json();
      }));
    }).then((data) => {
      let projects = this.createProjects(data[0].projects, data[1].tasklists, data[2].tasks);
